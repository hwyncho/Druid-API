...
router.get("/api/read/docs", (req, res) => {
  ...
  let subject = null;
  if (keys.includes("subject")) { subject = req.query.subject; }
  else { return res.status(400).json([]); }

  let limit = 10;
  if (keys.includes("limit")) { limit = parseInt(req.query.limit); }

  if (info.subjects.includes(subject)) {
    models[subject].find({}, projection)
      .sort({ date: -1 })
      .limit(limit)
      .exec((err, docs) => {
...
router.get("/api/read/doc", (req, res) => {
  ...
  let id = null;
  if (keys.includes("id")) { id = parseInt(req.query.id); }
  else { return res.status(400).json({}); }

  if (info.subjects.includes(subject)) {
    models[subject].findOneAndUpdate({ id: id }, { $inc: { click: 1 } }, { projection: projection })
      .exec((err, doc) => {
        if (err) { return res.status(500).json({}); }
        else {
